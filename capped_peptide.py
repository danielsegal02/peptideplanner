...
# When comparing to an online drawer I used it seems 2 very similar results were seen, Don't know if they are exactly identical though.
...
#NOTES: Capped peptide seeems to be the thing we need to help develop our app.
# We need to somehow integrate this, which I will be focusing on...
# It's the one we need, takes a BILN sequence, creates a sequence object and corrects the atoms,

#some input code

print('Enter your Peptide:')
xz = input()
print('Generating image for , ' + xz)
yz = ""
for i in range(0, len(xz)): #loop converts it to BILN format.
    yz += xz[i]
    if (i < (len(xz) - 1)):
        yz += "-"
print(yz) # prints format

biln = yz

#creates a seq object, then corrects the atom names.
 
seq = Sequence(biln)
# Correct atom names in the sequence object
seq = correct_pdb_atoms(seq)

#not entirley sure about this but I guess is useful in helping build the molecule I think
