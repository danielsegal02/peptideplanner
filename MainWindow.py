...
def on_button_click(event=None):
    ...
    #catch errors in input here... if anything is off
    dfx = pd.read_csv("AminoAcidTable.csv") # read csv
    inListy = dfx["Code"].tolist()
    amino_acid_string = amino_acid_string.rstrip(" ")
    for i in amino_acid_string:
        # if i is not in the input list... error is dispalyed and fucntion stops.
        if ((i in inListy) == False):
            label_e.config(text="Error: Bad Input! Please check legend, exclude spaces, and ensure input is valid.")
            return
    label_e.config(text = " ")
...
top_label = tk.Label(app, background="white", text="Welcome to the Peptide Planner!", font=("Roboto", 20))
top_label.pack(pady=10)

label_e = tk.Label(app, background="white", foreground= 'red', text=" ", font=("Roboto", 8))
label_e.pack(pady=10)
